...
static void create_depth_stencil_objects(uint32_t width, uint32_t height, uint32_t msaa_count,
    ...
    view_desc.Format =
        d3d.feature_level >= D3D_FEATURE_LEVEL_10_0 ? DXGI_FORMAT_D32_FLOAT : DXGI_FORMAT_D24_UNORM_S8_UINT;
    view_desc.Flags = 0;
    if (msaa_count > 1) {
    ...
    if (srv != nullptr) {
        ...
        srv_desc.Format =
            d3d.feature_level >= D3D_FEATURE_LEVEL_10_0 ? DXGI_FORMAT_R32_FLOAT : DXGI_FORMAT_R24_UNORM_X8_TYPELESS;
...
static void gfx_d3d11_init(void) {
    ...
    gfx_dxgi_create_factory_and_device(DEBUG_D3D, 11, [](IDXGIAdapter1* adapter, bool test_only) {
        ...
        HRESULT res = d3d.D3D11CreateDevice(adapter,
                                            ...
                                            D3D11_SDK_VERSION, test_only ? nullptr : d3d.device.GetAddressOf(),
                                            &d3d.feature_level, test_only ? nullptr : d3d.context.GetAddressOf());
    ...
    for (uint32_t sample_count = 1; sample_count <= D3D11_MAX_MULTISAMPLE_SAMPLE_COUNT; sample_count++) {
        ThrowIfFailed(d3d.device->CheckMultisampleQualityLevels(DXGI_FORMAT_R8G8B8A8_UNORM, sample_count,
                                                                &d3d.msaa_num_quality_levels[sample_count - 1]));
    }
...
void CSMain(uint3 DTid : SV_DispatchThreadID) {
    ...
    hr = d3d.D3DCompile(shader_source, strlen(shader_source), nullptr, nullptr, nullptr, "CSMain", "cs_4_0",
    ...
    ThrowIfFailed(d3d.device->CreateComputeShader(cs->GetBufferPointer(), cs->GetBufferSize(), nullptr,
                                                  d3d.compute_shader.GetAddressOf()));

    hr = d3d.D3DCompile(shader_source_msaa, strlen(shader_source_msaa), nullptr, nullptr, nullptr, "CSMain", "cs_4_1",
                        compile_flags, 0, d3d.compute_shader_msaa_blob.GetAddressOf(),
...
static struct ShaderProgram* gfx_d3d11_create_and_load_new_shader(uint64_t shader_id0, uint32_t shader_id1) {
    ...
    HRESULT hr = d3d.D3DCompile(buf, len, nullptr, nullptr, nullptr, "VSMain", "vs_4_0", compile_flags, 0,
    ...
    hr = d3d.D3DCompile(buf, len, nullptr, nullptr, nullptr, "PSMain", "ps_4_0", compile_flags, 0, ps.GetAddressOf(),
    ...
    ThrowIfFailed(d3d.device->CreateInputLayout(ied, ied_index, vs->GetBufferPointer(), vs->GetBufferSize(),
...
static void gfx_d3d11_upload_texture(const uint8_t* rgba32_buf, uint32_t width, uint32_t height) {
    ...
    ThrowIfFailed(
        d3d.device->CreateTexture2D(&texture_desc, &resource_data, texture_data->texture.ReleaseAndGetAddressOf()));

    // Create shader resource view from texture

    ThrowIfFailed(d3d.device->CreateShaderResourceView(texture_data->texture.Get(), nullptr,
...
static void gfx_d3d11_set_sampler_parameters(int tile, bool linear_filter, uint32_t cms, uint32_t cmt) {
    ...
    sampler_desc.Filter = linear_filter && d3d.current_filter_mode == FILTER_LINEAR ? D3D11_FILTER_MIN_MAG_MIP_LINEAR
...
static void gfx_d3d11_update_framebuffer_parameters(int fb_id, uint32_t width, uint32_t height, uint32_t msaa_level,
    ...
    if (diff || (fb.render_target_view.Get() != nullptr) != render_target) {
        ...
        if (render_target) {
            ThrowIfFailed(d3d.device->CreateRenderTargetView(tex.texture.Get(), nullptr,
    ...
    if (has_depth_buffer &&
        ...
        create_depth_stencil_objects(width, height, msaa_level, fb.depth_stencil_view.ReleaseAndGetAddressOf(),
...
void gfx_d3d11_start_draw_to_framebuffer(int fb_id, float noise_scale) {
    ...
    d3d.context->OMSetRenderTargets(1, fb.render_target_view.GetAddressOf(),
...
gfx_d3d11_get_pixel_depth(int fb_id, const std::set<std::pair<float, float>>& coordinates) {
    Framebuffer& fb = d3d.framebuffers[fb_id];
    TextureData& td = d3d.textures[fb.texture_id];
    ...
    if (coordinates.size() > d3d.coord_buffer_size) {
        ...
        ThrowIfFailed(d3d.device->CreateShaderResourceView(d3d.coord_buffer.Get(), &coord_buf_srv_desc,
                                                           d3d.coord_buffer_srv.GetAddressOf()));
        ...
        ThrowIfFailed(
            d3d.device->CreateBuffer(&output_buffer_desc, nullptr, d3d.depth_value_output_buffer.GetAddressOf()));
        ...
        ThrowIfFailed(d3d.device->CreateUnorderedAccessView(
            d3d.depth_value_output_buffer.Get(), &output_buffer_uav_desc, d3d.depth_value_output_uav.GetAddressOf()));
        ...
        ThrowIfFailed(
            d3d.device->CreateBuffer(&output_buffer_desc, nullptr, d3d.depth_value_output_buffer_copy.GetAddressOf()));
